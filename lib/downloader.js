...
const insta = (url) => new Promise((resolve, reject) => {
    ...
    fetchJson(`https://videfikri.com/api/igdl/?url=${url}`)
...
const fb = (url) => new Promise((resolve, reject) => {
        console.log(`Get Facebook media from ${url}`)
		 fetchJson(`http://docs-jojo.herokuapp.com/api/fb?url=${url}`)
        .then((result) => resolve(result))
	.catch((err) => reject(err))
})
/**
Facebook Video VideFriki
*/
const fbk = (url) => new Promise((resolve, reject) => {
        console.log(`Get Facebook media from ${url}`)
